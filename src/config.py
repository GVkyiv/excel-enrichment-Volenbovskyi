...
HTTP_TIMEOUT = 10  # секунд на звичайний запит
# Пакетний SPARQL це один запит за сотню сутностей, і публічний
# ендпоінт Wikidata відповідає на нього від часток секунди до хвилини.
# Заміряно на тому самому запиті: 0,3 с у теплому стані сервера і 60 с
# у холодному. З таймаутом 10 с пакет іноді зривався, і система йшла
# на запасний шлях через Nominatim по одному місту в секунду.
...
# Підказка про тип сутності з плану застосовується, лише поки типізований
# кандидат лишається співставно відомим. Поріг заміряний на двох реальних
# випадках: у «Madrid» типізований кандидат має 13 відсотків мовних розділів
# від справжнього Мадрида (підказка хибна, її треба ігнорувати), у
# «Annapurna» вершина має 28 відсотків від масиву (підказка правильна).
# Це компроміс між двома вимірами, а не універсальна константа.
